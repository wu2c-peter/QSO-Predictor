...
| `diagnostics/` | Doctors-framework core: findings types (`Severity`, `CheckResult`, `SettingsPanel`; re-exported by `audio_doctor.models`), detection layer (`probe_apps`/`probe_ports`/`setup_analysis`; `setup_wizard.py` is now Qt-only and imports it), and checkup machinery (`StationSnapshot`, `registry`, `report`). Audio Doctor's adapter is app-side (`audio_doctor/doctor.py`) per the one-way import rule. No Qt / app-module imports — enforced by `test_conventions.py`. Design: `DIAGNOSTICS_SPEC.md` |
