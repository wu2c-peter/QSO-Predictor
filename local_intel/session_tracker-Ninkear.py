...
class SessionTracker:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            model_manager: ModelManager for ML predictions (optional)
        """
        ...
        # Bayesian behavior predictor
        self._behavior_predictor = BehaviorPredictor(model_manager)
    ...
    def _handle_target_answer(self, decode: Decode, parsed: ParsedMessage):
        ...
        # Update Bayesian behavior predictor
        if self.target_session.answered_calls:
            last_answer = self.target_session.answered_calls[-1]
            pileup_snapshot = {
                call: {'snr': m.snr, 'freq': m.frequency}
                for call, m in self.target_session.callers.items()
            }
            self._behavior_predictor.update_with_observation(
                self.target_session.callsign,
                last_answer,
                pileup_snapshot
            )
    ...
    def get_target_behavior(self) -> Optional[Dict]:
        """
        Get target's observed behavior with Bayesian estimate.
        ...
        # Get live pattern analysis (requires 5+ QSOs)
        pattern = self._analyze_pattern()
        
        # Get Bayesian estimate (works even with 0 observations)
        bayesian = self._behavior_predictor.get_prior(session.callsign)
        logger.debug(f"get_target_behavior: {session.callsign} -> "
                    f"bayesian_style={bayesian.most_likely_style}, "
        ...
        return {
            ...
            'pattern': pattern,
            # Bayesian estimate - available immediately
            'bayesian_style': bayesian.most_likely_style,
    ...
    def clear_session(self):
        ...
        # End behavior tracking for current target (saves history)
        if self.target_session:
    ...
    def clear_all(self):
        ...
        # End all behavior sessions
        for callsign in self.active_sessions:
            self._behavior_predictor.end_session(callsign)
        self._behavior_predictor.clear_session()
    ...
    def reload_behavior_history(self):
        """Reload behavior history from disk (after bootstrap)."""
        self._behavior_predictor.reload_history()
    
    def get_behavior_stats(self) -> Dict:
        """Get behavior history statistics."""
        return self._behavior_predictor.get_history_stats()
