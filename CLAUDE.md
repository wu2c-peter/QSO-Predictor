...
| `diagnostics/` | Doctors-framework core: findings types (`Severity`, `CheckResult`, `SettingsPanel`; re-exported by `audio_doctor.models`), detection layer (`probe_apps`/`probe_ports`/`setup_analysis`, driven by `setup_wizard.py`), and checkup machinery (`StationSnapshot`, `registry`, `report`; Audio Doctor adapter is app-side in `audio_doctor/doctor.py`). No Qt / main-app imports — enforced by tests. Design: `dev-docs/DIAGNOSTICS_SPEC.md`. |
