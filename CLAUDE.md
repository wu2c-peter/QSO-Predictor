...
| `diagnostics/` | Doctors-framework core: findings types (`Severity`, `CheckResult`, `SettingsPanel`; re-exported by `audio_doctor.models`), detection layer (`probe_apps`/`probe_ports`/`setup_analysis`, driven by `setup_wizard.py`), checkup machinery (`StationSnapshot`, `registry`, `report`), and in-package doctors (`doctors/clock.py` + `probe_clock`). App-side pieces: Audio Doctor adapter in `audio_doctor/doctor.py`, driver in `controllers/diagnostics_controller.py`, results UI in `widgets/checkup_dialog.py`. No Qt / main-app imports in the package — enforced by tests. Design: `dev-docs/DIAGNOSTICS_SPEC.md`. |
