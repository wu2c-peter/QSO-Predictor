...
(The **Windows Fast Startup** check moved to the System Doctor in
v2.7.0 — run it from **Diagnostics → System Doctor...** or as part of a
Full Checkup.)
...
- **A fresh mute can hide from the audit at first.** Windows writes the persisted per-app store lazily, and the audit can only see a live session once WSJT-X has played audio since it started — so a mute set moments ago may not show in the audit until WSJT-X plays again. The **live TX path check always catches it**: press Tune and run it.
