...
[![Version](https://img.shields.io/badge/version-2.5.0-blue.svg)](https://github.com/wu2c-peter/qso-predictor/releases)
...
The scoring engine now uses **regional reporter consensus** to gauge confidence. When multiple PSK Reporter stations near the target all confirm a frequency slot is quiet, the score rises accordingly. One reporter's silence is a sample — five reporters' silence is a consensus.

* **Continuous confidence** — quiet slot scores scale smoothly from 50 (no reporters) to 82 (6+ reporters). No hard thresholds or cliffs.
* **Suspicious gap detection** — empty slots flanked by heavy target decoding activity are dampened, since the target's decoder is active nearby but finding nothing there (likely local QRM at their end).
* **Regional consensus recommendation** — when no proven Tier 1 data exists, the algorithm uses regional reporter data before falling back to blind gap-finding.

### Score Reason Tooltips

**Hover over the score graph** to see why any frequency has its current score — plain-English explanation of what data the system has and how it's scoring that slot. Examples: "Proven: 2 signal(s) decoded by target", "Regional quiet: 5 reporter(s) in area, clear", "Suspicious gap: flanked by 6 target decodes".
